<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column sortable align="left" label="动作" prop="action" width="120" />
        <el-table-column sortable align="left" label="相关人员" prop="user_id" width="120" />
        ...
        <el-table-column align="left" label="操作">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateHrmFinesOrBonusFunc(scope.row)">变更</el-button>
...
import {
  ...
  deleteHrmFinesOrBonus,
  deleteHrmFinesOrBonusByIds,
  updateHrmFinesOrBonus,
  findHrmFinesOrBonus,
  getHrmFinesOrBonusList
...
const getTableData = async() => {
  const table = await getHrmFinesOrBonusList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteHrmFinesOrBonusByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateHrmFinesOrBonusFunc = async(row) => {
    const res = await findHrmFinesOrBonus({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteHrmFinesOrBonusFunc = async (row) => {
    const res = await deleteHrmFinesOrBonus({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
